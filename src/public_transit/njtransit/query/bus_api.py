...
from public_transit.njtransit.query.bus_and_stop import NJTBusStop, NextBus, BUS_NUMBERS
...
def format_bus_message(stop: NJTBusStop, bus_arrivals: List[NextBus], direction: str) -> str:
    message = (f"To <b>{direction}</b> StopID={stop.id}:\n") + (
        "\n".join([bus.to_html() for bus in bus_arrivals]) if bus_arrivals else "No bus found"
    )
    return message


async def extract_content_from_page(stop: NJTBusStop, busno: str, browser: Browser) -> BeautifulSoup:
...
def parse_content_from_page(stop: NJTBusStop, page_content: BeautifulSoup) -> List[NextBus]:
